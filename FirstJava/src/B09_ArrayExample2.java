...
public class B09_ArrayExample2 {

	public static void main(String[] args) {
		// 배열
		// 같은 타입의 변수를 한번ㄴ 여러개 선언하는 방법
		...
		System.out.println("welcome 배열의 길이: "+welcome.length); //length: 요소의 갯수
...
//		char[5] s2;		// 배열 선언시 오류...[5] 숫자의 의미는 배열의 길이로 생각하고 쓸수 없어요.
		 ...
		 */
		System.out.println("\n강화된 for를 이용한 반복 처리...");
		for (String str : welcome) { //welcome은 String[]이기 때문에 Strinf str로 받게됨
		...
		// 배열의 복사
		// 배열의 얕은 복사(shallow copy)와 같은 복사(deep copy)
...
//		System.out.println(Arrays.toString(byeMessage2));
		...
		// 2. 깊은 복사(Deep copy)
		// System.arraycopy(stc, stcPos, dest, destpos, length)
		// src : 원본 배열의 이름
		// srcPos : 복사를 시작할 원본의 위치(인덱스)
		// dest : 붙여넣기 할 배열의 이름
		// destPos : 붙여넣기를 시작할 위치를 설치(인덱스)
		// length : 복사를 몇개나 할 것인지 결정!!
		
		System.out.println("복사전 : "+Arrays.toString(byeMessage));
		System.arraycopy(byeMessage2, 1, byeMessage, 5, 2);
		
		System.out.println("복사후 : "+Arrays.toString(byeMessage));
		System.out.println("복사후 : "+Arrays.toString(byeMessage2));
