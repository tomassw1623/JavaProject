 ...
 * 	- 여기 Usage Modifier(사용 제한자) 중 static, final, native 등이 있음.
 * 	- 이외에 transient, synchronized, volatile, native 등이 있음.
 * 
 * ## static의 의미...
 * 	- 일반적으로 각 인스턴스들은 서로 독립적으로 객체 내에서 변수값을 서로 각각 다름.
 * 	- Person p1, Person p2 가 있음 경우에 각각의 p1.getName(), p2.getName()는 다른 값을
 * 	  가질 수 있음.
 * 	- static이 변수 앞에 붙으면 모든 인스턴스에 대해서 공유가 됩니다. 이때 변수는 인스턴스 변수가
 * 	 아닌 클래스 단위로 바뀌게 되고, 이 때문에 static이 붙은 맴버 변수를 클래스 변수라고 합니다.
 * 그리고, static 붙지 않는 변수를 인스턴스 변수라고 하며 구분합니다.
 *  - 메서드 static이 붙었다면 해당 메서드는 인스턴스와 무관하기 때문에 인스턴스를 생성하지 않아도
 *   호출이 가능하게 된다.
 *   
 *  -- static 관련 된 자바 문법
 *   > static 메서드 안에서는 non-static 맴버를 객체 생성 없이 직접 참조할 수 없음.
 *   > 반대로 static 붙지 않은 메서드에서는 static 맴버를 사용할 수 있음.
 *   
...
public class C06_static {
	...
	public static void main(String[] args) {
		...
		// static method
		System.out.println("Counr.doIt () : "+Count.doIt());
		System.out.println("Counr.doIt () : "+Count.doIt());
		System.out.println("Counr.doIt () : "+Count.doIt());
	}
