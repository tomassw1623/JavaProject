...
public class C02_method2 {
	...
	public static void main(String[] args) {
		 ...
		 *  - 매개변수도 변수의 일종으로 데이터 타입과 함께 선언해서 사용해야 합니다. 단, 초기화 하지 않음
		 *  - 매개변수를 여러개 선언할 수 있고, 메서드가 외부로부터 받을 데이터가 없다면 선언하지 않아도 됨.
		 *  그리고, 매개 변수가 없다면 반환 유형 처럼 void는 사용하지 않고, 소괄호로 안을 비워 둡니다.
		 *  
		 *  ** 반환 유형(return type)
		 *  - 반환 유형은 메서드가 실행한 후에 메서드를 호출한 곳에서 반환하는 값을 전달하는 데이터...
		 *  - 메서드는 반환값이 있을 수 있고, 없을 수도 있는데 없는 경우에는 반환 유형에 "void"를 사용합니다.
		 *  - 반환값이 있는 메서드를 호출할 때는 해당 반환 유형에 맞는 변수에 반환값을 지정해야 합니다.
		 *  - 메서드 실행에 경과값을 반환할 때는 return 키워드를 사용합니다.
		 */
