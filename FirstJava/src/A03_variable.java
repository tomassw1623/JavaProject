public class A03_variable {
    ...
    public static void main(String[] args) {
        ...
        // 변수, 메서드, 클래스 이름을 식별자
        // - 대문자와 소문자를 구분하소 , 길이 제한이 없음. ex) Run, rUn, ruN
        // - 문자와 숫자, 밑줄(_), 달러기호($)를 포함할 수 있습니다.
        //     문자는 영문자만 가리키는 것이 아니라 영문자 사용을 추천
        // _, $는 로컬 시스템에서 규칙이 있는 경우가 있기 때문에 부득이안 경우가 아니면 사용 안함.
        // - 문자, 밑줄(_), 달러기호($)로 시작할 수 있음. 숫자로 시작할 수 없음.
        // - 식별자는 공백을 포함할 수 없습니다. ex) is child(x) -> ischild, is_child
        // - 키워드는 식별자로 사용하면 안됩니다.
        // - 키워드는 시스템 또는 자바에서 기본적으로 사용하고있는 명령어나 변수들을 의마함...
        ...
        a = 20; // 같은 이름의 변수는 밑에서 다시 선언할 수 없어요.
        // ** int는 정수 타입을 나타내는 키워드

        // 변수를 사용하는 이유...
        // 1. 값을 일과적으로 변경할 수 있게 하기 위해서...(같은 값을 여러번 사용해야 하는 경우 편리함)
        int k = 11;
        ...
        // 2. 값에 의미를 부여할 수 있습니다.(이 때문에 작명 규칙이 중요함)
        int height = 180;
        int subject = 5;
        int old = 26;
        int weight = 70;
