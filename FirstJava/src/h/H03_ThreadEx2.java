...
public class H03_ThreadEx2 {
	 ...
	 * 			InterrunptedException처리를 강제하기 때문에 try구문 사용
	 * 	- interrupt() : 스레드가 일시 정지 상태일 있을 때에 InterrunptedException예외발생
	 * 				run()메서드가 정상종료 시킬 수 있게 한다.
	 * 	- join() : 다른 스레드 실행이 완료될때까지 기다리는 메서드 
	 * 	- yield() : 다른 스레드에세 실행 양보하는 메서드
	 */
...
//					}
		...
		t4.isContimue = false;	// t4 스레드 yield(). 즉, 양보
	
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {}
		
		t4.isContimue = true;	// t4 다시 실행
